                ...
                <v-col cols="12">
                  ...
                  >
                    <v-text-field
                      ...
                      required
                      v-model="selectedItemInternal.employeeName"
...
export default {
  ...
  methods: {
    ...mapActions('employeeStore', ['onCreateItem', 'onUpdateItem']),
    async createOrUpdateItem() {
      ...
      try {
        const employee = {
          employeeName: this.selectedItemInternal.employeeName
        };
        if (this.selectedItem) {
          employee.id = this.selectedItem.id;
          const selectedIdx = this.employees.indexOf(this.selectedItem);
          await this.onUpdateItem({ idx: selectedIdx, item: employee });
        } else {
          await this.onCreateItem(employee);
        }
        this.dialog = false;
      } catch (err) {

    ...
    <template v-slot:activator="{ on: dialog }">
      <v-tooltip bottom>
        <template v-slot:activator="{ on: tooltip }">
          <v-btn dark color="blue darken-1" v-on="{ ...dialog, ...tooltip }">
            Внести изменения
          </v-btn>
        </template>
...
export default {
  ...
  methods: {
    async createOrUpdateItem() {
      ...
      try {
        let creatingStocktaking = [];
        this.allItems.forEach(element => {
          if (
            element.changedSheets &&
            element.changedSheets.trim().length > 0
          ) {
            creatingStocktaking.push({
              inventoryCardId: element.id,
              docNumber: this.selectedItemInternal.docNumber,
              dateChanging: new Date(
                this.selectedItemInternal.dateChanging
              ).toISOString(),
              changedSheets: element.changedSheets
            });
          }
        });
        if (creatingStocktaking.length > 0) {
          const response = await repository.bulkCreate(creatingStocktaking);
        }
